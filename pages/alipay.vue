<template>
  <div class="wxPay wrapper_1200">
    <div class="header">
      <span class="home">首页 > 个人中心 > 确认订单 ></span> 订单号：{{
        orderId
      }}
    </div>
    <div class="wrapper">
      <div class="title acea-row row-between-wrapper">
        <div>支付宝支付</div>
        <div class="amount">
          应付金额：<span class="num font-color">{{ totalPrice }}</span
          >元
        </div>
      </div>
      <div class="acea-row row-center-wrapper">
        ...
        <div class="phone"><img src="../assets/images/alipay.png" /></div>
      </div>
      <div class="selectPay" @click="Return">
...
export default {
  ...
  head() {
    return {
      title: "支付宝支付-" + this.$store.state.titleCon,
    };
  },
  created() {
  ...
  methods: {
    aliSuccess() {
      let that = this;
      that.$axios
        .get("/pc/check_order_status/" + that.orderId + "/" + that.invalid)
        .then((res) => {
    ...
    goAli() {
      ...
      that.$axios
        ...
        .then((res) => {
          that.codeUrl = res.data.result.jsConfig.qrCode;
